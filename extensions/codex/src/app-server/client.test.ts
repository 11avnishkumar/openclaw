...
describe("CodexAppServerClient", () => {
  ...
  it("initializes with the required client version", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "openclaw/0.124.0 (macOS; test)" },
    });
  ...
  it("blocks unsupported app-server versions during initialize", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "openclaw/0.123.9 (macOS; test)" },
    });

    await expect(initializing).rejects.toThrow(
      `Codex app-server ${MIN_CODEX_APP_SERVER_VERSION} or newer is required, but detected 0.123.9`,
    );
    expect(harness.writes).toHaveLength(1);
  });
  ...
  it("blocks same-version Codex app-server prereleases below the stable floor", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "openclaw/0.124.0-alpha.2 (macOS; test)" },
    });

    await expect(initializing).rejects.toThrow(
      `Codex app-server ${MIN_CODEX_APP_SERVER_VERSION} or newer is required, but detected 0.124.0-alpha.2`,
    );
    expect(harness.writes).toHaveLength(1);
  });
  ...
  it("accepts newer Codex app-server prereleases", async () => {
    ...
    harness.send({
      id: outbound.id,
      result: { userAgent: "openclaw/0.125.0-alpha.1 (macOS; test)" },
    });
  ...
  it("reads the Codex version from the app-server user agent", () => {
    expect(readCodexVersionFromUserAgent("Codex Desktop/0.124.0")).toBe("0.124.0");
    expect(readCodexVersionFromUserAgent("openclaw/0.124.0 (macOS; test)")).toBe("0.124.0");
    ...
    expect(readCodexVersionFromUserAgent("openclaw/0.124.0abc")).toBeUndefined();
